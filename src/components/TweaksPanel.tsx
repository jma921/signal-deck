...
export function useTweaks(defaults: TweakValues): [TweakValues, (key: keyof TweakValues, value: TweakValues[keyof TweakValues]) => void] {
  const [values, setValues] = useState(defaults);
  const setTweak = useCallback((key: keyof TweakValues, value: TweakValues[keyof TweakValues]) => {
    setValues((prev) => ({ ...prev, [key]: value }));
    // no-op postMessage for design host compatibility
...
export function TweaksPanel({ title = "Tweaks", children }: { title?: string; children?: React.ReactNode }) {
  ...
  // also allow toggling with keyboard shortcut ` (backtick)
  useEffect(() => {
